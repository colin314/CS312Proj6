...
class Proj5GUI:
    ...
    def solveClicked(self, nPts=15, itCount=100, sVal=100, f_Tmax=lambda n,i: 5*n,seed=20):                                # need to reset display??? and say "processing..." at bottom???
        ...
        annealCost = result['cost']
        annealTime = result['time']
        pImp = np.abs(greedyCost- annealCost)/greedyCost*100
        # sol_y = result['sol_y']
        ...
        randTime = 0#result['time']
        randCost = greedyCost#result['cost']
        #result = self.solver.branchAndBound(10)
        BBTime =  0#result['time']
        BBCost = 0#result['cost']
...
gui = Proj5GUI()    
nPts = [15,20,25,30,40,50,60,100,150,200]
itCount = [100]
